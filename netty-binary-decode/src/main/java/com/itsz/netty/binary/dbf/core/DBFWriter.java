...
public class DBFWriter extends DBFBase implements java.io.Closeable {
	...
	public void updateOneRecord(Object[] values,int index ) throws DBFException{
		...
		if (this.raf == null) {
			throw new DBFException("no records in file");
		} else {
			try {
				 int seekIndex = header.headerLength+index*header.recordLength;
				 raf.seek(seekIndex);
				 writeRecord(this.raf, values);
				//this.header.write(this.raf);
				 //this.raf.seek(this.raf.length());
